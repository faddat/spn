...
docker buildx build --tag spn:arm64 --file .pi/Dockerfile --platform linux/arm64 --cache-from faddat/spn:cache --cache-to faddat/spn:cache --load --progress tty .
...
sudo bash -c "echo spn > ./.tmp/result-rootfs/etc/hostname"
...
sudo losetup --find --show images/spn.img
